    ...
    for item in chunk:
        ...
        if domain is not None:
            ...
            history = History(
                ...
                domain_data["category_group"],
                item["url"],
                domain,
                domain_data["category_description"],
                int(item["lastVisitTime"]),
            )
            visit = Visits(
                slug, domain_data["category"], domain, int(item["lastVisitTime"])
            )
            visit.save()
            history.save()
...
def create_pending_card(self, result, slug):
    ...
    try:
        history_response = process_user_history(
            result, slug, user.get("first_time_user", False)
        )
        schedule_next_execution(
            result, slug, user.get("first_time_user", False), hours=23, minutes=0
        )
...
def schedule_next_execution(result, slug, first_time_user, hours, minutes):
    next_execution = datetime.now(timezone.utc) + timedelta(
        hours=hours, minutes=minutes
    )
    async_result = create_pending_card.apply_async(
        kwargs={"result": result, "slug": slug}, eta=next_execution
    )

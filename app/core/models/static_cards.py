...
class StaticCards:
    ...
    def save(self):
        ...
        existing_card = find_static_cards_by_slug(
            self.document["slug"], self.document["type"]
        )
        if existing_card:
            update_static_card(
                self.document["slug"],
...
def update_static_card(slug, type, metadata, card_id=None):
    try:
        ...
        user_of_db = db.static_cards.find_one_and_update(
            filter_query,
            update_operation,
...
def delete_static_card(slug, ids):
    ...
    if result.deleted_count > 0:
        return (
            jsonify(
                {
                    "message": f"{result.deleted_count} static card(s) belonging to user {slug} deleted successfully"
                }
            ),
            200,
        )
        ...
        return (
            jsonify({"error": f"No cards found with the provided IDs for user {slug}"}),
            404,
        )

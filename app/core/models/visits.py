...
class Visits():
    def __init__(self, slug, category, domain, visitTime, create_timestamp = int(datetime.now().timestamp())):
    ...
    def save(self):
        if find_by_slug_and_time(self.document['slug'], self.document['visitTime'], self.document['domain']):
            return

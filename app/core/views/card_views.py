    ...
    try:
        ...
        if is_publish_card:
            published_card = PublishedCard(
                slug,
                tobe_published_card["cardType"],
                tobe_published_card["content"],
                tobe_published_card["tags"],
                tobe_published_card["urls"],
                tobe_published_card["metadata"],
                tobe_published_card["category"],
                tobe_published_card["date"],
            )
            published_card.save()
        delete = delete_pending_card(
            slug, ObjectId(id)
        )  # We protect user privacy by deleting pending cards once we create published cards
        update_last_cards_marked(slug)
        update_tobe_release_kleo_token(slug)
        update_kleo_points_for_user(slug)
...
def mint_published_card(slug, **kwargs):
    try:
        ...
        return (
            jsonify(
                {"error": f"error while marking published card as minted for {slug}"}
            ),
            500,
        )
...
def get_adjacent_published_cards(slug, **kwargs):
    try:
        ...
        try:
            ...
            return (
                jsonify(
                    {"error": "Invalid date format. Use epoch timestamp (integer)"}
                ),
                400,
            )

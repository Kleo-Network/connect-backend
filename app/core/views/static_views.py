...
def create_calendly_cards(slug, **kwargs):
    try:
        ...
        static_card = StaticCards(
            slug,
            "CalendarCard",
            int(datetime.now().timestamp()),
            metadata={"slug": user_slug},
        )
        static_card.save()

        return (
            jsonify({"message": f"Created static calendly card for user {slug}"}),
            200,
        )
...
def create_github_cards(slug, **kwargs):
    try:
        ...
        token_response = requests.post(
            GIHUB_AUTH_TOKEN_URL,
            json=git_auth_params,
        ...
        if not access_token:
            return (
                jsonify({"error": "error while fetching access token of github"}),
                500,
            )
        ...
        if not contribution:
            return (
                jsonify(
                    {
                        "error": f"Error while fetching contribution from github for {slug}"
                    }
                ),
                400,
            )
...
def create_insta_cards(slug, **kwargs):
    try:
        ...
        if not token:
            return (
                jsonify({"error": "error while fetching access token of instagram"}),
                500,
            )
            # Construct GraphQL query
        ...
        if response.status_code == 200:
            ...
            if media_data:
                ...
                for media in media_data[:max_photos]:
                    ...
                    try:
                        ...
                        obj = {"url": image_url, "caption": media["caption"]}
            ...
            return jsonify(
                {"error": f"Failed to fetch media data from Instagram for user {slug}"}
            )
...
def create_x_cards(slug, **kwargs):
    try:
        ...
        response = requests.post(
            "https://api.twitter.com/2/oauth2/token", data=payload, headers=headers
        )
        ...
        if user_response.status_code == 200:
            ...
            pinned_tweet = (
                user_data.get("includes", {}).get("tweets", [{}])[0].get("text", "")
            )
            ...
            static_card = StaticCards(
                slug, "XCard", int(datetime.now().timestamp()), metadata=x_meta_data
            )
            static_card.save()
